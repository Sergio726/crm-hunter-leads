...
| WEB-26 | Investigar glitch visual (estática/ruido de colores) al entrar a `/clientes` en mobile real | 🟠 mitigación aplicada (2026-07-11) | Confirmado por el usuario en vivo (celular real, no solo foto), pasa apenas entra. Se descartó: blur sin proteger (`md:backdrop-blur`), hydration mismatch (sin warnings en consola), FOUC de tema (ya tiene script anti-flash). No reproducido en local/dev a 390px. **Nueva hipótesis investigada**: Tailwind v4 compila los modificadores de opacidad sobre tokens oklch (`bg-primary/10`, etc.) a `color-mix(in oklab, ...)` calculado en runtime; `Badge.tsx` usaba ese patrón en sus 6 tonos y `ClientsTable.tsx` monta un `Badge` por cada fila apenas se pinta la pantalla — decenas de `color-mix()` de golpe en el primer paint, patrón conocido por causar banding/ruido en GPUs Android con drivers viejos. **Fix aplicado** (no confirmado, es hipótesis): tokens `--badge-*-bg` en `globals.css` (`:root`/`.dark`) con alfa oklch literal en vez de modificador `/N`, usados en `Badge.tsx` — elimina el `color-mix()` en el elemento que se repite por fila, sin cambiar el resultado visual. `tsc`/`build` OK. Otros usos de `/N` en `ClientsTable.tsx` (barra de selección múltiple) y `ClientDrawer.tsx` quedaron sin tocar porque solo se renderizan tras interacción, no en el primer paint. **Pendiente: confirmación del usuario en su celular real** — si el glitch persiste, sigue haciendo falta el video de pantalla para ver el instante exacto |
