...
_Última actualización: 2026-09-06 (**Rediseño UX/UI móvil de Turbo** en `feat/turbo-ui-motion`: login, carga, tabs, tarjetas, búsquedas, vacíos, motion y estados de Turbo renovados con la identidad ST Labs; sin assets de marca duplicados. Falta abrir en Expo Go/dispositivo real. La compilación automática quedó pendiente por un bloqueo local de npm en Windows durante la restauración de `mobile/node_modules`. — Estado previo 2026-08-14: backend propio + módulo de prospección.)_
...
3. **Nuevo rediseño móvil Turbo (APP-12)**: abrir `mobile/` en Expo Go y recorrer login, carga, Pendientes, Clientes, Contactados y ficha. Confirmar contraste del mint, animaciones y objetivos táctiles en un teléfono real. Antes, resolver la restauración incompleta de `mobile/node_modules` causada por bloqueo de Windows para poder correr `tsc`/`expo export`.
