...
| PERM-3 | Adjuntar **fotos, PDFs y notas de voz** al seguimiento | 🟡 web hecho (2026-07-11) | Migración `0019`. Web: `SellerClientDrawer.tsx` — cada ítem del historial tiene un botón "Adjuntar" (límite 10MB, `image/*`\|`application/pdf`\|`audio/*`) que sube a Storage e inserta la fila de metadata, y lista los adjuntos existentes con un botón "Ver" que genera una URL firmada (`createSignedUrl`, 5 min — el bucket es privado). Insert de metadata probado contra Supabase real (transacción revertida). `tsc`/`build` OK. **Sin UI en la app**: requeriría instalar `expo-image-picker` (fotos/PDF) y `expo-av`/`expo-audio` (notas de voz) — dependencias nativas nuevas que no se pueden probar sin un dispositivo real, se dejan para una sesión donde se pueda validar en el device. **Sin comprimir imágenes** antes de subir (queda para cuando se resuelva la app, que es donde más importa por el tamaño de fotos de cámara) |
...
| 2 — Pantallas | ✅ hecho (2026-07-11), salvo un pendiente — Mi perfil · APP-4 (editar cliente) · comentario rápido · WEB-18 (invitar con rol): completos en app y web. Adjuntos en ficha: completo en **web**, **falta la app** (necesita `expo-image-picker`/`expo-av`, dependencias nuevas sin poder probar en dispositivo) | `mobile-app` y `web-admin` en paralelo | Sprint 1 |
