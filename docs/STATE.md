...
0. **Aplicar migración `0027_crm_sync_enabled.sql` en Supabase Cloud** (`CRM.LITE` / `rtvvamemdhbvmyxtxonb`) — SQL ya en el repo; el MCP de esta sesión no tenía acceso a ese proyecto. Tras aplicarla, Configuración → "Sincronización con GHL" queda operativo (D12).
