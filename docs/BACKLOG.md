...
| Fecha | Tarea |
|---|---|
| 2026-07-10 | **UXR-1 + UXR-2**: login muestra el error de OAuth (`login/page.tsx` lee `?error=auth`, envuelto en `Suspense`); fix de clases de color rotas en el logo de respaldo (`brand/Logo.tsx`). `tsc`/`lint`/`build` OK (lint tiene 9 errores preexistentes en otros archivos no tocados, regla `react-hooks/set-state-in-effect`, fuera de alcance). Se armó también un botón "Hacer admin" para WEB-17 en esta rama, pero se **descartó**: otra rama en paralelo (`worktree-purrfect-pondering-bengio`) ya lo había resuelto de forma más completa (ascender + bajar rol, confirmación, guard SQL) — se prefirió esa implementación, ver nota en la fila WEB-17 más arriba |
