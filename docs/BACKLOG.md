...
| 4 — Notificaciones | 🟡 backend hecho + workflows escritos (2026-07-12) — NOTIF-1. Migración `0025` aplicada y verificada; `notify-user.json`/`notify-overdue.json` versionados. Falta: deploy n8n + `n8n_notify_url` + testear mensajería GHL (necesita el usuario) | `integrations-n8n` + `backend-supabase` | Sprint 1/2 (teléfono del vendedor) |
...
| Fecha | Tarea |
|---|---|
| 2026-07-12 | **Sprint 4 — Notificaciones (NOTIF-1), backend + workflows**: migración `0025_notify_user.sql` aplicada a cloud y verificada e2e (trigger `lead.assigned` + RPC `n8n_list_overdue_followups`/`n8n_mark_notified` + tabla `notifications` con dedupe diario). SAFE-BY-DEFAULT (inerte sin `n8n_notify_url`). Probado: trigger genera 1 fila (transacción revertida, nada persistió), RPC devuelve payload normalizado con dato real, rechaza secreto inválido, advisors solo con el patrón `n8n_*` intencional. Workflows `notify-user.json`/`notify-overdue.json` escritos y sumados a `deploy-workflows.ps1`; contrato documentado en `docs/INTEGRACION-N8N.md`. Pendiente del usuario: deploy n8n + setear `n8n_notify_url` + testear mensajería GHL |
