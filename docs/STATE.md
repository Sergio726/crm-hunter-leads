...
1. **Sprint 4 — Notificaciones** (`NOTIF-1`): backend ✅ hecho y verificado (migración `0025`). **Falta el usuario**: (a) desplegar `notify-user.json`/`notify-overdue.json` a n8n con `.\n8n\deploy-workflows.ps1` (necesita `apikeyn8n`), (b) setear `app_settings.n8n_notify_url = https://<instancia>/webhook/crm-notify`, (c) verificar/testear la mensajería GHL (Conversations API — versión de header + forma del body sin confirmar, ver `docs/INTEGRACION-N8N.md`) y que GHL tenga canal SMS/WhatsApp/email configurado. Recién ahí queda activo. Siguiente en el plan: **Sprint 5 — Semáforo (SEM-1/2)**.
