...
| WEB-17 | **Convertir vendedor existente en administrador** | ✅ hecho (2026-07-10) | web-admin | Botón "Hacer admin" en la tabla de Vendedores (`TeamManager.tsx`), llama al RPC `set_user_role` ya existente. `tsc`/`lint`/`build` OK |
...
| WEB-17 | **Convertir vendedor existente en administrador** | ✅ hecho (parcial) | Corrección 2026-07-10: **no hace falta RPC nuevo** — ya existe `public.set_user_role(target_user uuid, new_role text)` desde la migración `0001` (superadmin-only, valida `'seller'`/`'superadmin'`). Falta solo UI: en "Administradores" (Configuración) o en `TeamManager`, un selector para elegir un vendedor existente y botón "Convertir en admin" que llame `supabase.rpc('set_user_role', ...)`. Caso puntual ya resuelto a mano por SQL: `soporte@justmore.net` promovido a `superadmin` el 2026-07-10 |
...
| Fecha | Tarea |
|---|---|
| 2026-07-10 | **WEB-17 + UXR-1 + UXR-2**: botón "Hacer admin" en Equipo (`TeamManager.tsx`, usa el RPC `set_user_role` ya existente); login muestra el error de OAuth (`login/page.tsx` lee `?error=auth`, envuelto en `Suspense`); fix de clases de color rotas en el logo de respaldo (`brand/Logo.tsx`). `tsc`/`lint`/`build` OK (lint tiene 9 errores preexistentes en otros archivos no tocados, regla `react-hooks/set-state-in-effect`, fuera de alcance de esta tarea) |
