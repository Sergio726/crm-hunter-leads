...
_Última actualización: 2026-07-17 (**Sesión NOTIF/CONT/APP**: (1) **NOTIF-1 en producción** verificado e2e — workflows activos en `moremigracion.com`, corregido el `429 too many requests` de GHL con reintento+backoff (falta redeploy o toggle manual para el ajuste fino 5/5s), envío GHL probado (upsert+mensaje 201). (2) **CONT-1 cerrado** (D11): el 2º teléfono/email NO se sincroniza a GHL, vive solo en CRM Lite. (3) **APP-6 hecho**: tab "Clientes" con buscador en la app (ver todos, no solo pendientes) — `tsc`+`expo export` OK, sin probar en dispositivo. (4) **WEB-6/UXR-6 hecho**: dashboard admin con tendencia diaria (recharts `TrendChart`), deltas semanales en StatCards y feed de actividad (`ActivityFeed`) — `tsc` limpio + `next build` verde, sin probar en navegador (login Google). (5) **WEB-7/UXR-7 hecho**: badges de pendientes/vencidos en el sidebar (`AppShell` los trae por client supabase, RLS por rol) + card "Vencidos" y tonos de urgencia en el dashboard (`StatCard` con prop `tone`). `tsc` + `next build` verde. (6) **WEB-27 + WEB-27b hechos**: vista Kanban de clientes (`/clientes` toggle Tabla/Tablero) con drag & drop — surgió del pedido de Alejo (embudo accionable) + mockup aprobado. Mejoras tras revisión: mover a Contactado registra interacción + seguimiento (`BoardMoveDialog`), `<select>` de estado por tarjeta (táctil), Deshacer + confirmar cierres, scroll/paginación por columna + filtro vendedor. `tsc` + `next build` verde, sin probar en navegador. Todo en el **PR #12**. Detalle NOTIF-1 abajo: — Previo 2026-07-12: **Sprint 4 — Notificaciones NOTIF-1**: backend hecho y verificado e2e — migración `0025_notify_user.sql` aplicada a cloud, trigger `lead.assigned` + RPC de vencidos + tabla `notifications`, todo SAFE-BY-DEFAULT (inerte sin `n8n_notify_url`). Workflows `notify-user.json`/`notify-overdue.json` escritos y versionados. **Falta el usuario**: desplegar los workflows n8n (`deploy-workflows.ps1`, necesita `apikeyn8n`), setear `n8n_notify_url`, y testear la mensajería GHL (no se pudo desde el worktree sin credenciales). Ver `docs/INTEGRACION-N8N.md`. — Sesión previa 2026-07-11: deployado a `somosmore/main` a pedido explícito del usuario, sin mergear el PR #6 en `origin`. **`origin/main` y `somosmore/main` quedaron desincronizados a propósito** — `somosmore/main` ya tiene el trabajo de esta sesión (paginación WEB-8 + fix WEB-26), `origin/main` todavía no. El PR #6 en `origin` sigue abierto; conviene mergearlo en algún momento para que no diverjan más, o cerrarlo si se decide que `somosmore` es la fuente de verdad de acá en más. housekeeping + WEB-26/WEB-8: el usuario mandó un video del glitch real (`bug cel.mp4`), se analizó frame a frame con ffmpeg — la causa más probable no es CSS/color sino que `/clientes` dibujaba las 46 filas sin paginar y eso traba el scroll en su celular. Se paginó `ClientsTable.tsx` (20 por tanda). Ver detalle completo en `docs/BACKLOG.md` (WEB-8, WEB-26). **Falta que el usuario confirme en su celular real** si el glitch desapareció — no se pudo probar interactivo en esta sesión porque el login pide Google OAuth real. Sigue pendiente además: Sprint 3 completo `/vendedor` eliminado, panel unificado por rol con `AppShell`/`ClientDrawer`/`ClientsTable` únicos. Migraciones `0015`→`0024`. Nada de eso probado en sesión real todavía)_
