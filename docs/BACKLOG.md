...
| ID | Tarea | Prioridad | Notas |
|---|---|---|---|
| NOTE-1 | Comentario rápido (nota libre) en el seguimiento | 🟡 backend hecho y probado (2026-07-11) | Migración `0020_note_channel.sql` + fix `0022_fix_note_outcome_check.sql`. **Bug encontrado probando 0020**: el constraint original (`channel <> 'note' or outcome is null`) solo bloqueaba en una dirección — permitía canales no-`'note'` (whatsapp/sms/email/call) con `outcome` nulo, cuando debían seguir exigiéndolo. Corregido a un constraint simétrico (`(channel = 'note') = (outcome is null)`) y verificado con transacciones de prueba revertidas: nota sin outcome OK, nota con outcome bloqueada, whatsapp sin outcome bloqueado. Tipos TS actualizados; se ajustaron 4 pantallas que indexaban `OUTCOME_LABELS[outcome]` sin chequear null. **Falta (Sprint 2)**: la UI — botón/ícono de "comentario rápido" en la tarjeta del cliente (app) y en la ficha (web) que abre solo un cuadro de texto |
